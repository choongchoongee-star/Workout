...
try {
  ...
  await addBench()
  assert.equal(await picker(cards.nth(0)).inputValue(), 'unspecified')
  await picker(cards.nth(0)).selectOption('barbell')
  ...
  assert.equal(await picker(cards.nth(3)).inputValue(), 'unspecified')
  await picker(cards.nth(3)).selectOption('smith')
  ...
  assert.equal(await progressEquipment.inputValue(), 'unspecified')
  assert.equal(await progressEquipment.locator('option').count(), 6)
  await progressEquipment.selectOption('smith')
  ...
  await page.getByRole('button', { name: '+ Add exercise', exact: true }).click()
  const modal = page.getByRole('dialog', { name: 'Add exercise', exact: true })
  await modal.getByRole('button', { name: 'All', exact: true }).click()
  await modal.getByPlaceholder('Search exercises...').fill('Treadmill')
  await modal.getByText('Treadmill', { exact: true }).click()
  const cardio = page.getByRole('group', { name: 'Treadmill', exact: true })
  const selector = cardio.first().getByLabel('Equipment for Treadmill')
  assert.equal(await selector.inputValue(), 'unspecified')
  assert.equal(await selector.locator('option').count(), 6)
  await selector.selectOption('barbell')
  assert.equal(await cardio.count(), 2)
  await cardio.last().locator('input[type=number]').first().fill('10')
  await page.waitForTimeout(800)
  await page.reload()
  await cardio.last().waitFor()
  assert.equal(await cardio.last().locator('input[type=number]').first().inputValue(), '10')
  assert.equal(await cardio.last().getByLabel('Equipment for Treadmill').inputValue(), 'barbell')
