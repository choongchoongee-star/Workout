...
│ ┌────────────────────────────────────┐│
│ │ Bench Press                  Edit  ││  ← Edit 진입 시 삭제 기능 표시
│ │ Chest                              ││
│ │ Set      kg       Reps       Done  ││  ← 열 제목은 카드당 한 번 표시
│ │  1   ┌──80───┐ ┌──10───┐   [✓]   ││  ← 입력 영역과 완료 영역 분리
│ │      │-2.5│+2.5│ │-1│+1│           ││  ← 숫자 아래 증감 버튼
│ │  2   ...                           ││
...
- **세트 행 밀도와 조작 구분:** 일반 글자 크기에서 행 높이 약 70px. 카드당 한 번 `Set / kg(lbs) / Reps / Done` 열 제목을 표시한다. 각 행은 세트 번호, 무게 입력기, 횟수 입력기, 완료의 4열이다. 각 입력기는 숫자를 위에, 32px 높이의 감소/증가 버튼을 아래에 묶는다. 숫자는 16px이며 전체 선택·직접 입력을 유지한다. 오른쪽 완료 영역은 여백·세로 구분선으로 입력기와 분리하고 44×44px 버튼을 사용한다. 완료된 세트는 녹색 번호와 체크, 잠긴 입력기로 표시한다. 320/375/390/430px 너비 × 667px 높이 브라우저에서 한 운동의 5세트가 하단 탭 위에 모두 보이며 가로 넘침이 없음을 검증했다. 키보드가 닫힌 기본 화면 기준이며 실기기 safe area와 확대 글자는 별도 확인 대상이다.
...
- **편집·삭제:** 운동 카드의 `Edit`으로 편집 모드에 진입하고 `Done`으로 종료한다. 편집 중 세트 입력은 잠기고 완료 버튼 대신 빨간 휴지통 삭제 버튼이 나타난다. 일반 기록 모드에는 삭제 버튼을 표시하지 않아 완료와 삭제가 나란히 있지 않다. 편집 모드 카드 하단에 `Delete exercise`를 표시하며 유산소 운동 삭제도 이 경로를 사용한다. 삭제 → 즉시 제거 + UndoToast(5초), 되돌리기 시 원위치 복원. 세트 추가는 두 모드에서 유지한다.
