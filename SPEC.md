...
> 스토어 준비 현황 (2026-09-08): ASC 이름 Steady Sets/부제 Simple Workout Log, 건강 및 피트니스, 영문 소개·키워드·지원/마케팅/개인정보 URL·심사 메모·로그인 불필요·수동 출시 저장 완료. 연령 설문 저장 결과 한국 전체/대부분 지역 9+. 전 세계 175개 국가/지역 및 향후 지역 사용 가능, 타사 콘텐츠 없음, 사용자 확인 후 의료 기기 아님을 저장했다. 개인정보는 GitHub Pages 보안용 IP 저장을 기타 데이터/앱 기능/사용자 연결/추적 없음으로 초안 저장했으며, 최종 게시의 정확성·법규 준수 동의는 사용자 확인 대기다. 사용자가 제공한 심사 연락처 전체를 저장하고 재로드 확인했다. 개인 값은 Git에 기록하지 않는다. 개인정보 초안 게시 승인 후에도 자동 승인 검토가 실행 직전 동의를 요구하여 최종 게시를 차단했으며 확인 창에서 대기한다. 앱 심사 제출과 최종 출시는 명시적으로 보류한다. 저작권·가격·스크린샷·최종 이름/아이콘 포함 새 빌드도 대기 중이다. 세부 사항은 store/RELEASE.md에 기록하며 심사 연락처는 Git에 보관하지 않는다.
