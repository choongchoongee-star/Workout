...
│ ┌────────────────────────────────────┐│
│ │ Bench Press · Chest        Delete ││  ← 실제 UI는 큰 카드 배경 없는 구분선 목록
│ │   #     kg (±2.5)    Reps    Done  ││  ← 열 제목은 운동당 한 번 표시
...
- **세트 행 밀도와 조작 구분:** 일반 글자 크기에서 행 높이 33px. 큰 카드 배경과 상자형 입력기를 사용하지 않고 얇은 구분선으로 운동·행을 나눈다. 운동 이름·카테고리는 한 줄, 열 제목은 운동당 한 번 `# / kg (±2.5)` 또는 `lbs (±5) / Reps / Done`으로 표시한다. 각 행은 왼쪽 휴지통, 세트 번호, 무게 입력기, 횟수 입력기, 오른쪽 완료의 5열이다. 입력기는 감소·숫자·증가를 가로 한 줄로 배치하며 버튼에는 −/+ 기호를 사용하고 접근성 이름과 title에 증감량을 명시한다. 숫자는 16px이며 전체 선택·직접 입력을 유지한다. 완료 버튼은 36×32px 클릭 영역 안에 20px 체크를 두고 세로 구분선으로 입력과 분리한다. 삭제는 반대쪽 끝에 항상 표시하며 번호 열이 무게 입력과의 간격을 확보한다. 완료된 세트는 녹색 번호와 체크, 잠긴 입력기로 표시한다. 320/375/390/430px 너비 × 667px 높이 브라우저에서 운동 2개 × 5세트, 총 10개 행이 하단 탭 위에 모두 보이며 가로 넘침이 없음을 검증했다. 키보드가 닫힌 기본 화면 기준이며 실기기 safe area와 확대 글자는 별도 확인 대상이다.
...
- **삭제:** 각 행 맨 왼쪽의 휴지통으로 세트를 바로 삭제한다. 운동 전체 삭제는 제목 오른쪽 `Delete` 버튼이며 유산소에도 동일하다. 편집 모드나 숨겨진 삭제 버튼은 없다. 삭제 → 즉시 제거 + UndoToast(5초), 되돌리기 시 값·완료 상태·순서를 복원한다. `+ Add set`은 운동 목록 바로 아래의 얇은 텍스트 버튼이다.
...
- **스테퍼 간격:** 각 열 중앙에 무게 최대 100px, 횟수 최대 80px의 고정 폭 조절 영역을 둔다. 양쪽 버튼은 각각 24px, 가운데는 숫자 입력이며 숫자 길이가 바뀌어도 버튼 위치를 이동하지 않는다. 화면이 좁으면 열 너비에 맞춰 축소한다. 33px 행 높이와 2개 운동 × 5세트 표시를 유지한다.
