...
- **세트 행 밀도와 조작 구분:** 일반 글자 크기에서 행 높이 33px. 운동명 16px, 숫자 기본 18px(6자 이상은 16px), 번호와 Add set 14px, 열 제목 12px. 운동 분류는 핵심 기록 목록에서 생략한다. 열은 Del. / Set / kg(lbs) / Reps / Done이며, 왼쪽 32×32px 휴지통과 오른쪽 44×32px 완료 클릭 영역을 분리한다. 숫자 버튼은 밑줄로 편집 가능함을 표시하고 열 너비를 클릭 영역으로 사용한다. 완료되면 초록 체크·번호와 잠긴 숫자를 표시한다. 320/375/390/430px 너비 × 667px 높이 브라우저에서 운동 2개 × 5세트가 하단 탭 위에 모두 보이고 가로 넘침이 없음을 검증했다. 10세트 표시는 키보드가 닫힌 기본 글자 크기 기준이다. 확대 시 스크롤을 허용하며 실기기 확인은 별도다.
