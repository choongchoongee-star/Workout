...
mode !== 'capacitor' && VitePWA({
  ...
  manifest: {
    ...
    theme_color: '#171916', background_color: '#171916',
...
│   │   ├── StepperInput.jsx  # 숫자 표시 버튼 + 큰 숫자 조절 dialog
...
- 활성 탭 `text-accent-400`, 비활성 `text-zinc-500`.
...
- **숫자 수정:** 목록의 밑줄 숫자를 누르면 해당 운동명·세트 번호·Weight/Added weight/Reps가 표시된 native dialog를 연다. 목록에는 반복 ± 버튼을 표시하지 않는다. 창에는 56px ± 버튼, 30px 숫자 직접 입력, 단위, 48px 이상 Cancel/Apply 버튼이 있다. kg 증감 2.5 / lbs 증감 5 / reps 증감 1. 입력을 누르면 전체 선택한다. Apply만 값을 반영하고 Cancel·Escape·창 배경 클릭은 초안을 버린다. 빈 값·유한하지 않은 값·최솟값 미만은 적용할 수 없다. dialog가 열릴 때 제목에 포커스를 두어 키보드를 자동 표시하지 않는다. 완료 세트의 숫자 수정은 잠긴다.
- **세트 행 밀도와 조작 구분:** 일반 글자 크기에서 행 높이 33px. 운동명 16px, 숫자 18px, 번호와 Add set 14px, 열 제목 12px. 운동 분류는 핵심 기록 목록에서 생략한다. 열은 Del. / Set / kg(lbs) / Reps / Done이며, 왼쪽 32×32px 휴지통과 오른쪽 44×32px 완료 클릭 영역을 분리한다. 숫자 버튼은 밑줄로 편집 가능함을 표시하고 열 너비를 클릭 영역으로 사용한다. 완료되면 라임 체크·번호와 잠긴 숫자를 표시한다. 320/375/390/430px 너비 × 667px 높이 브라우저에서 운동 2개 × 5세트가 하단 탭 위에 모두 보이고 가로 넘침이 없음을 검증했다. 10세트 표시는 키보드가 닫힌 기본 글자 크기 기준이다. 확대 시 스크롤을 허용하며 실기기 확인은 별도다.
...
- **삭제:** 각 행 맨 왼쪽의 휴지통으로 세트를 바로 삭제한다. 운동 전체 삭제는 제목 오른쪽 `Delete` 버튼이며 유산소에도 동일하다. 편집 모드나 숨겨진 삭제 버튼은 없다. 삭제 → 즉시 제거 + UndoToast(10초), 되돌리기 시 값·완료 상태·순서를 복원한다. `+ Add set`은 운동 목록 바로 아래의 얇은 텍스트 버튼이다.
...
- **커스텀 운동 시각 구별:** `id.startsWith('custom-')`이면 `bg-accent-950/50 border border-accent-800/50` + 라임 "Custom" 라벨 + 삭제(×) 버튼. **이 색상 구별은 라이브러리 화면에서만.** 운동 기록 추가(Session) 모달 리스트는 색 구별 없이 그대로.
...
- **테마:** 차콜·오프화이트 다크 고정, `color-scheme: dark`. `src/index.css`의 Tailwind `@theme`에서 앱 전체 색상을 정의한다. 배경 `zinc-950 #171916 / 900 #20231e / 800 #2d312a`, 선 `700 #454b40`, 보조 텍스트 `600 #858b7d / 500 #969d8d / 400 #acb2a4 / 300 #c7cbbf / 200 #daddd2`, 본문 `white #eeeee7`. 웹 preview manifest도 배경색을 공유한다.
- **포인트 컬러:** 낮은 채도 라임 `accent-600 #b4cc78`(주요 버튼·선택 배경, 글자는 `text-zinc-950`), `accent-400 #c2d68b`(활성 탭·링크·완료), `accent-500 #acc568`(진행 바·포커스). 보조 토큰 `300 #d2dfab / 700 #9bb55e / 800 #4d5d35 / 950 #29311e`. 순수 파랑 강조 대신 같은 토큰을 모든 앱 화면과 휴식 타이머 SVG에 사용한다.
- **상태색:** 세트 완료는 라임 체크·번호와 입력 잠금, 일반 성공·알림 허용은 `green`, 위험/삭제는 `red-400/500`, 에러 배너는 `red-900/30 + red-800`.
...
- **숫자 조절과 확대:** 기록 목록은 숫자 버튼만 표시하고 큰 ± 조절은 dialog에 모은다. dialog는 최대 384px, 작은 화면에서는 좌우 16px 여백을 두며 native focus trap과 Escape를 지원한다. viewport의 maximum-scale/user-scalable 제한을 제거해 사용자 확대를 허용한다. 200% 루트 글자 크기에서 조절창 열기·닫기를 브라우저로 확인했다.
