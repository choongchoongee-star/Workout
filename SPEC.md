...
| `wl_equipment_[familyId]` | 구형 마지막 선택 장비 | 미사용 | 기존 키는 보존하되 새 카드 기본값에 사용하지 않음 |
...
│ │ 1       80          10      [✓] ││  ← 왼쪽 스와이프 시 오른쪽 Delete 노출
│ │ ──────────────────────────────── ││  ← 화면 높이에 맞춘 행, 얇은 구분선
│ │        −  +        −  + ││
│ │ ...                               ││
...
- **삭제:** 세트 번호 아래 ×는 제거한다. 행을 왼쪽으로 밀면 오른쪽에 폭 88px·최소 높이 44px의 Delete 버튼이 나타나며, 버튼을 눌러야 삭제한다. 전체 스와이프로 즉시 삭제하지 않는다. 번호·상시 ±·완료 체크·행 높이는 유지한다. 운동 전체 삭제는 제목 오른쪽 `Delete` 버튼이며 유산소에도 동일하다. 삭제 → 즉시 제거 + UndoToast(10초), 되돌리기 시 값·완료 상태·순서를 복원한다. `+ Add set`은 운동 목록 바로 아래의 얇은 텍스트 버튼이다.
...
- History 날짜 카드를 왼쪽으로 밀면 오른쪽 Delete 버튼이 나타난다. 탭은 기존 상세 이동이며 스와이프 후 클릭은 차단한다. Delete는 해당 날짜 세션 전체를 삭제하고 목록에서 10초 Undo를 제공한다. 상세 화면의 기존 Delete도 유지한다.
...
- 2026-09-06: 세트 왼쪽 열을 번호 위·× 삭제 아래 순서로 변경했다. 삭제 및 증감 동작과 크기는 유지한다.

- 2026-09-06: 5세트 운동 영역 목표를 화면의 약 80%에서 60%로 줄였다. 숫자·± 크기는 유지하면서 행 사이 세로 여백을 축소하고 작은 화면의 최소 조작 높이는 보존한다.
...
- 2026-09-06: 사용자 승인으로 다섯 장비 분류와 구형 EZ Bar → Barbell 기록 통합(소스 42ff518)을 OTA 게시했다. runtime `ios-edab217484237bd7`, bundle `f60b97696e11bb8f1c94c50baa2092e6dca11a02c56794c63c36c2207bff1b2e`, ZIP 384648 bytes. native fingerprint·iOS 호환 검사, 장비/OTA 테스트 14개, lint·build 및 공개 manifest/ZIP SHA-256·RSA 검증을 통과했다. Settings → Check for updates에서 다운로드 후 완전 종료·재실행으로 적용한다. 새 네이티브 빌드는 실행하지 않았으며 실제 iPhone 적용 확인은 별도다.
...
- 2026-09-07: 아이콘 양쪽 큰 원판 바깥에 작은 살구색 원판을 한 개씩 추가해 각 측면 두 장 구조로 변경했다. 바를 소폭 연장하고 원판 사이 간격으로 각 원판을 구분한다. SVG·favicon·PWA 192/512·iOS 1024 산출물을 동기화했다. 배포는 별도다.

- 2026-09-07: 아이콘의 큰 원판과 바깥 작은 원판 사이 간격을 SVG 192 기준 4에서 2로 절반 축소했다. 작은 원판만 안쪽으로 이동하고 큰 원판·바·색상은 유지한다. iOS/PWA/favicon 산출물을 재생성했다.

- 2026-09-07: 사용자가 실기기 확인 완료를 보고했다. 스크린샷을 제외한 출시 준비 요청으로 Steady Sets 영문 스토어 메타데이터(store/metadata-en-US.json)와 진행표(store/RELEASE.md)를 작성하고 필드 길이를 검증했다. 정확히 같은 이름의 운동 앱은 공개 검색에서 찾지 못했으나 Steady 유사 앱이 있어 스토어 저장/상표 확인과 구분한다. 새 이름·아이콘·지원 앵커·백업 안내와 Live Activities 개인정보 설명을 공개 사이트에 배포하고 실제 URL 및 기존 OTA manifest HTTP 200을 확인했다. lint·웹 빌드·사이트 빌드를 통과했다. App Store Connect 브라우저 로그인, 가격/국가 선택, 공개 지원 이메일은 사용자 입력 대기이며 원격 이름·메타데이터·연령·개인정보·가격 설정은 아직 저장하지 않았다. 신규 EAS 빌드·정식 심사 제출은 실행하지 않았다.
...
- 2026-09-08: 언어 지원 추가 검증으로 scripts/verify-language-flows.mjs를 추가했다. 격리된 Edge 브라우저에서 ko-KR/en-US 입력·숫자 편집·재로드·장비별 기록·직접 등록한 운동 분류·맨몸/유산소 저장·타이머 탭 왕복·기록/세트 삭제와 되돌리기·실제 포인터 스와이프·백업 내보내기/중복 방지/삭제 후 복원/손상 파일 거부를 통과했다. 언어 변경 시 동일 저장 데이터와 설정 보존, fr-FR 영어 fallback, 320/375/390/430px 버튼 가림·가로 넘침 및 라이트/다크 탭 가림 검사를 통과했다. 저장 상태 대조 26회, 전체 단위 테스트 60개(한국어/영어 native 알림 mock 포함), SSR·iOS 구성 검사를 통과했다. 테스트 스크린샷은 임시 폴더에만 보관한다. 브라우저 검사는 OS 언어 선호를 locale로 모사하며 실제 iPhone Preferred Language 메뉴·Swift 컴파일·Dynamic Island 표시를 대체하지 않는다. EAS 실행은 없었다.
...
- 2026-09-09: 승인된 빌드 8(72f57bb9-e725-4b12-a04e-1fdf36b56062)이 ERRORED로 종료되었고 IPA는 생성되지 않았다. 웹 자산 빌드와 Capacitor 동기화는 통과했지만, EAS가 app.json/App 번호를 올린 후 위젯 번호 동기화 전에 전체 검사(verify-live-activity)가 실행되어 7 != 8로 중단됐다. 원격에서는 동기화 직후 언어 리소스 검사만 실행하고, 앱/위젯 버전 검사는 eas/configure_ios_version 뒤로 배치한다. 전체 check:ios-release는 로컬 사전 검사로 유지한다. 격리된 빌드 폴더에서 7/8 실패를 재현하고 언어 검사 통과 및 번호 동기화 후 앱/위젯 검사 통과를 확인했다. 원본 빌드 번호도 8로 맞췄다. lint·웹 빌드·iOS 구성 검사 통과. 재빌드는 실행하지 않았다.

- 2026-09-09: 사용자 승인 재빌드 9(8a635822-6364-4fda-ae8d-89467e97f3bd, 기반 ec67d38)가 ERRORED로 종료됐다. EAS configure_ios_version 성공 후에도 custom 위젯 CURRENT_PROJECT_VERSION이 8에 머물러 app.json 9와 불일치했다. 언어 리소스 검사·의존성 설치·웹 빌드·Capacitor sync·서명 설정은 서버에서 통과했으며 fastlane archive와 IPA 생성은 실행되지 않았다. scripts/sync-ios-versions.mjs를 버전 설정 뒤/위젯 검사 앞에 실행하여 app.json의 buildNumber를 앱·위젯 Debug/Release 전체에 명시적으로 적용한다. 숫자 검증과 타깃 수 방어, 파일 포맷 및 OTA hash 정규화 보존을 포함한다. 실제 실패한 빌드 9 체크아웃에서 8 != 9 실패 → 동기화 스크립트 → 앱/위젯 및 언어 검사 통과를 확인했다. 단위/동기화 테스트 총 62개, lint·웹 빌드·로컬 iOS 구성 검사 통과. 원본 buildNumber는 9로 동기화했다. 추가 EAS 재빌드·제출·출시는 실행하지 않았다.

- 2026-09-09: 사용자 승인으로 빌드 10(3e5bd2ce-0ef6-45d9-a2bf-ca2ccc5d3293, 기반 304735f, runtime ios-df63450d92c09d12)을 실행해 Build finished/IPA 생성을 확인했다. 서버에서 한국어/영어 리소스 검사와 명시적 앱·위젯 번호 동기화/검증을 통과했다. 실제 IPA를 임시 폴더에서 검사하여 App.app 및 RestTimerActivity.appex의 CFBundleVersion=10, CFBundleLocalizations=en/ko, 각 en.lproj/InfoPlist.strings 및 ko.lproj/InfoPlist.strings 존재와 표시명을 확인했다. 원본 app.json/모든 CURRENT_PROJECT_VERSION을 10으로 동기화하고 로컬 iOS 구성 검사 통과. ASC 업로드·Apple 언어 목록 처리 확인·심사 제출·최종 출시는 실행하지 않았다.

- 2026-09-09: 사용자 승인으로 빌드 10(3e5bd2ce-0ef6-45d9-a2bf-ca2ccc5d3293)을 App Store Connect에 업로드했다. Submission 4415d835-b36c-4853-9f98-0fddcb2e704c가 성공 종료했으며 CLI에서 binary successfully uploaded와 Apple processing 상태를 확인했다. 사전 TestFlight 그룹 조회는 인증 오류로 건너뛰었지만 실제 업로드는 성공했다. Apple 처리 완료·한국어 지원 언어 표시·심사 빌드 교체·정식 심사 제출·최종 출시는 아직 확인/실행하지 않았다.
