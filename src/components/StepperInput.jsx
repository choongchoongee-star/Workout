...
function NumberEditor({ value, onChange, step, unit, min, label, contextLabel, onClose }) {
  ...
  return (
    <dialog ref={dialogRef} aria-label={`${contextLabel} ${t(label)}`}
      ...
      <form onSubmit={e => { e.preventDefault(); if (valid) { onChange(number); onClose() } }}>
        ...
        <h2 ref={headingRef} tabIndex={-1} className="mt-1 text-xl font-semibold outline-none">{t(label)}</h2>
        <p className="mt-2 text-sm text-zinc-300">{t("Use − / +, or tap the number to type.")}</p>
        <div className="my-5 grid grid-cols-[3.5rem_minmax(0,1fr)_3.5rem] items-center gap-3">
          <button type="button" aria-label={t('Decrease {unit} by {step}', { unit: t(unit), step })} onClick={() => adjust(-1)}
            className="h-14 rounded-lg border border-zinc-600 text-2xl active:bg-zinc-800">−</button>
          <div className="min-w-0 text-center">
            <input type="number" inputMode="decimal" step="any" min={min} aria-label={t(unit)} value={draft}
            ...
            <span className="text-sm text-zinc-300">{t(unit)}</span>
          </div>
          <button type="button" aria-label={t('Increase {unit} by {step}', { unit: t(unit), step })} onClick={() => adjust(1)}
            className="h-14 rounded-lg border border-zinc-600 text-2xl active:bg-zinc-800">+</button>
        </div>
        {!valid && <p role="alert" className="mb-3 text-sm text-red-300">{t('Enter a number of {min} or more.', { min })}</p>}
        <div className="grid grid-cols-2 gap-3">
          <button type="button" onClick={onClose} className="min-h-12 rounded-lg border border-zinc-600 text-base active:bg-zinc-800">{t("Cancel")}</button>
          <button type="submit" disabled={!valid} className="min-h-12 rounded-lg bg-accent-600 text-base font-semibold text-zinc-950 active:bg-accent-700 disabled:opacity-40">{t("Apply")}</button>
        </div>
      </form>
    </dialog>
  )
    ...
    <>
      <div className="grid w-full max-w-28 min-w-0 grid-cols-[2.75rem_minmax(0,1fr)_2.75rem] items-center justify-self-center">
        ...
        <button type="button" disabled={disabled} onClick={() => setEditing(true)}
          aria-label={t('Edit {label}: {value} {unit}', { label: t(label).toLowerCase(), value, unit: t(unit) })} aria-haspopup="dialog"
      ...
      {editing && <NumberEditor value={value} onChange={onChange} step={step} unit={t(unit)} min={min}
        label={t(label)} contextLabel={contextLabel} onClose={() => setEditing(false)} />}
    </>
  )
}
